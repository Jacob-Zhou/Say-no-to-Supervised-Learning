...
class LSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers=1, dropout=0):
        ...
        for _ in range(self.num_layers):
            self.cells.append(nn.LSTMCell(input_size=input_size,
                                            hidden_size=hidden_size))
    ...
    def forward(self, sequence, hx=None):
        ...
        if hx is None:
            ih = x.new_zeros(self.num_layers, batch_size, self.hidden_size)
            h, c = ih, ih
        else:
            h, c = self.permute_hidden(hx, sequence.sorted_indices)
        ...
        for i in range(self.num_layers):
            ...
            if self.training:
                mask = SharedDropout.get_mask(x[0], self.dropout)
                x = [i * mask[:len(i)] for i in x]
            x, (h_i, c_i) = self.layer_forward(x=x,
                                               hx=(h[i], c[i]),
                                               cell=self.cells[i],
                                               batch_sizes=batch_sizes)
            h_n.append(h_i.unsqueeze(0))
            c_n.append(c_i.unsqueeze(0))
        x = PackedSequence(x,
                           sequence.batch_sizes,
                           sequence.sorted_indices,
                           sequence.unsorted_indices)
